...
void solve() {
    ...
    for (ll l=0; l<=n-3; l++) {
        for (ll r=l+1; r<=n-2; r++) {
            ll sum1 = prefixSum[l+1] - prefixSum[0];
            ll sum2 = prefixSum[r+1] - prefixSum[l+1];
            ll sum3 = prefixSum[n] - prefixSum[r+1];

            if (sum1 % 3 == sum2 % 3 && sum2 % 3 == sum3 % 3) {
                cout << l+1 << " " << r+1 << "\n";
                return;
            } 
            else if (sum1 % 3 != sum2 % 3 && sum1 % 3 != sum3 % 3 && sum2 % 3 != sum3 % 3) {
                cout << l+1 << " " << r+1 << "\n";
                return;
            }
        }
